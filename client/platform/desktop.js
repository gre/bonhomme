...
Desktop.prototype = {
  ...
  _reaskName: function(e) {
    e.preventDefault();
    var self = this;
    prompt("Change your name:", getLocalName()).then(function(name) {
  ...
  setViewportPos: function(x, y) {
    ...
    this.bar.style[transformKey] =
      "translate3D(" + x + "px," + (y - 40) + "px,0)";
  },
  setGridY: function(y) {
  ...
  createRenderer: function() {
    var renderer = PIXI.autoDetectRenderer(conf.WIDTH, conf.HEIGHT, {
  ...
  onResize: function() {
    ...
    var y = Math.max(
      0,
      Math.min(Math.round((window.innerHeight - conf.HEIGHT) / 2), 120)
    );
